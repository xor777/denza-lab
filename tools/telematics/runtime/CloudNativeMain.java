...
public final class CloudNativeMain {
    public static void main(String[] args){
        try{
            if(args.length==3 && args[0].equals("control")){
                ...
                new CloudNativeBridge(null,marker,args[2],false).serve(System.in,System.out);return;
            }
            if(args.length==3 && args[0].equals("recover")){
                ...
                CloudNativeGuardian.runRecovery(Paths.get(args[1]),args[2]);return;
            }
            if(args.length==4 && args[0].equals("bridge")){
                ...
                CloudInstallMarker.read(marker);
                new CloudNativeBridge(workdir,marker,args[3]).serve(System.in,System.out);return;
            }
            if(args.length==3 && args[0].equals("guardian")){
                requireStartPermit();
                Path workdir=workdir(args[1]),marker=Paths.get(args[2]);
                CloudNativeGuardian.run(workdir,marker);return;
            }
    ...
    private static void runWorker(Path workdir,String nonce)throws Exception{
            ...
            Thread control=new Thread(()->{
                try{new CloudRuntimeProtocol(supervisor,android.os.Process.myPid()).serve(System.in,System.out,nonce);}
