...
internal class FakeShell(
    ...
    /**
     * Машинная правда 2026-08-28, включаемая явно: корень 4 фоновых задач не держит, и ВИДИМОЕ
     * окно, выселенное туда `am stack move-task <id> 4 false`, встаёт поверх сцены - area 4
     * (инцидент владельца 2026-08-27). Точное условие («что именно и в каком такте») живьём не
     * снято, поэтому по умолчанию фикстура остаётся на измеренном частном случае (`toTop`), а
     * этот флаг включает накрытие для сценария, который его лечит.
     */
    var evictionCoversTheScene = false
    ...
    fun shell(command: String): String {
        ...
        return when {
            ...
            command.startsWith("am stack move-task ") -> {
                ...
                if (rootId == FULL_ROOT && (toTop || evictionCoversTheScene)) area = 4
