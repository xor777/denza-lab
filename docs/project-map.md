...
| `feature.split` | Explicit «Разделить экран» launcher plus two standalone, resizeable Denza picker tasks, one per BYD root. Exact `START_IVI_PRIMARY` / `START_IVI_SECOND` categories create and target the native roots without transaction 115; unresizeable Launcher3/SR bootstrap tasks are not retained as hosts. A pane-local accessibility overlay blocks the stock Launcher's short interactive window during native drag-control reopen, then disappears after the Denza picker is confirmed. A picker tap moves exactly the chosen app above that picker, including Denza Apps itself. `MainActivity` follows ordinary task/Back behavior and never calls the split coordinator or mutates its persisted slots. Native configuration changes and visible-picker callbacks reconcile the automaton with live root/task topology because BYD can swap app tasks between stable picker hosts during divider resize; a reproduced boundary case where both picker tasks coalesce in one root and the app-only root is misclassified remains open. The last pair and automaton ownership are persisted, and task cleanup is component-validated through the local-ADB shell helper. Direct category placement, picker dismissal/reopen, basic divider movement, toggle shutdown, and standalone-picker navigation projection/return are live-proven. Simulcast still needs its own focused cross-feature acceptance pass. |
