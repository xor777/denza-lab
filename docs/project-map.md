...
| `feature.mirrors` | Migrated AVC renderer and stock-window monitor. Uses the shared local ADB client, keeps verified Mirrors geometry/image treatment, and has no probe dependency. It consumes confirmed mode and transient raw onset events through the demand-driven vehicle-signal hub. A raw onset can preempt an active opposite-side surface. A repeated onset for the already starting/showing side is retained only inside the same uninterrupted gesture; raw neutral or confirmed `OFF` closes that gesture, so a new same-side activation also tears down the old surface. A later same-epoch confirmed side plus the first exact matching stock-window observation may reopen after full vendor teardown. Unknown/unavailable input requires neutral. Ordinary right cycles, including two that reproduced the trailing same-side onset, and a stationary direct left-to-right canary are live-accepted; the exact transient-ambiguity wait branch has unit coverage but did not recur in the final live run. |
