...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation and HUD guidance, explicit stock-IVI split sessions, a swipeable bottom panel (trip/spectrum plus vehicle instruments), native weather adaptation, passenger-screen app installation, and a hidden stock-settings Russian locale toggle. | **Active.** Version `0.5.4`; one installed APK contains both the ordinary Denza Apps launcher and the toggle-controlled Split Screen alias plus its picker/host. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, trip inputs, native weather refresh, the former contextual split route, and FSE installation have been exercised on the author's car. One-APK split startup, saved-pair restore, native divider movement, toggle shutdown, standalone-picker navigation projection/return, ordinary Denza Apps task/Back behavior, post-reboot accessibility recovery, and Home-to-fullscreen app isolation are live-verified. The vehicle-instrument page and Russian locale toggle are locally verified but not yet exercised on the car. The processed front-DVR renderer remains compiled but product-disabled; rapid left-to-right mirror switching remains unsafe. |
