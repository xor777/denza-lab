...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation and HUD guidance, explicit stock-IVI split sessions, Contour instruments (drawn as Luminofor since 2026-09-23), a bottom trip/spectrum strip, native weather adaptation, passenger-screen app installation, Shortcuts default-app role selection, a hidden stock-settings Russian locale toggle, and the car's cloud link over Wi-Fi («Облако», built 2026-09-23, not yet on the car). | **Active.** Version `0.6.1`; one installed APK contains the ordinary Denza Apps launcher and the toggle-controlled Split Screen alias and picker. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, trip inputs, native weather refresh, the former contextual split route, and FSE installation have been exercised on the author's car. The rebuilt Split Screen core was partially accepted live on 2026-09-18; see "A narrated live session, mapped to the logs (2026-09-18)" in [split-screen-findings.md](split-screen-findings.md), and section 12 of [split-screen-product-contract.md](split-screen-product-contract.md) for the protocol that is still owed. Shortcuts roles store the selected application directly; update recovery is deferred after the single-package navigation proxy failed live routing. The latest Mirrors cancellation fix and startup-notification change have focused live checks; the wider reversal/same-side/stress matrix remains open. Signal-hub delivery hardening has host-only validation. See [instrument-display-findings.md](instrument-display-findings.md) and [vehicle-data-findings.md](vehicle-data-findings.md) for the exact build boundaries. |
...
None of the `experiments/` modules below is in the default Gradle build; each is
configured only under the `experiments` property, as "Build Outputs" describes.
...
| Component | Status |
| --- | --- |
| `MainActivity`, `ui/DenzaAppsScreen` | Adaptive Compose shell with twelve dashboard tiles, including the `Приложения` entry for navigation, music and video defaults used by stock Shortcuts. Its panel reads and writes the three AutoVoice PersonBean roles directly through `ContentResolver` (live-proven 2026-09-03), and every role stores the selected application's real package. There is no default-app runtime proxy or background role listener. Every installed launcher remains selectable, but media command compatibility still depends on the application's firmware media integration. Fullscreen keeps the original landscape dashboard; the measured 2/3 pane exposes that dashboard through horizontal scrolling, while the measured 1/3 pane stacks every tile in one vertical list. Choosing an application is one chooser page everywhere: a panel shows what is chosen on a row and turns into the grid when the row is pressed (`tools/design-canvas/README.md`, "Choosing an application"); the grid fits what fits in the narrow pane, and the trip spectrum spans the pane above vertically stacked trip figures. Below the tiles sits the trip strip, whose **field has two pages and whose dots under it say so**: the analyser with the track and the trip's figures, and the car's own readings — what the pack is doing, its voltage, five temperatures against the cluster's own thresholds, the engine, the trip, and the last ten kilometres as a line (`docs/energy-display-contract.md`). A horizontal swipe anywhere on the strip moves between them and the choice is remembered. Tiles, panes and strip are drawn to the Luminofor boards (`tools/design-canvas/luminofor/`, approved 2026-09-23). The car's page claims `VehicleTelemetryHub` only while it is on screen. This is not the return of `BottomPanelPager`, which had four pages nobody could see and was deleted on 2026-08-27: two pages, one gesture, and an indicator that is drawn whether or not anybody swipes (`tools/design-canvas/README.md`, "The strip is two pages"). Settings use standard Material 3 switches, segmented selectors, and action buttons; the selected Simulcast apps are a strip of their icons on the panel's row, and «Экран справа» opens its chooser from both gestures on the tile. The driver's-screen tile includes an opt-in steering-wheel control: every ★ press independently requests one immediate action, with no double-press behavior. Its panel names the choice on a row and «Что показывать» is a page: this app's own instruments (`Приборы`) under one heading and every application the car can open under the other — no package list since 2026-09-23. The instruments have one placement, so the panel drops its placement row instead of showing three dead cells. Attention states show a concrete instruction and reuse existing action slots; technical diagnostics remain hidden in Help. |
...
| `feature.speaker` | The Devialet pop-out covers, as one sentence said to the car. `SpeakerCoverService` watches media sessions and eagerly-opening players, asks `SpeakerCoverPolicy` whether this is a moment to speak, and `SpeakerCoverProtocol` writes the single playback report `INSTRUMENT_MUSIC_STATE_SET` (`0x43E0000A`, device `1007`) = `1` through the shell, because the app's own UID is refused at the property layer. It never touches the motor and never touches the car's stock auto-lift flag `AUDIO_RLSA_STATE_SET` (`0x16300025`) — `SpeakerCoverFlagContractTest` fails the build if the package so much as names it. The panel's one «Поднять» button answers whether or not the automation is switched on. Normative source: "Product contract v2" in [speaker-lift-findings.md](speaker-lift-findings.md). |
| `feature.cloud` | The «Облако» tile: the car's own cloud client (`cloudmanager`) kept online over ordinary internet, so the official Denza phone app sees the car. `CloudLinkService` watches the default network and `CloudLinkController` translates usable internet (Wi-Fi; or validated mobile data with actual stock APN state guarded, built for owners to test and unproven) for the stock client through the shell - the `double_apn` profile, `notify_nw(4)` while there is internet and `-5` when it has stayed gone - with `CloudLinkCore` deciding when, including bounded repeats after the stock receiver closes the gate itself. A second switch is the car's own `byd_off_wifi_switch` (Wi-Fi kept on through ACC-off), read back from the car. Off never touches the car on its own. Built and unit-tested on 2026-09-23; not yet installed or live-tested. Contract: "Denza Apps «Облако» tile" in [telematics-findings.md](telematics-findings.md). |
| `feature.media` | The steering wheel's Play/Pause key, answered directly instead of by the firmware's routing. `MediaResumeCore` is the pure policy and `MediaResumeController` the Android half: it holds a target per live session token, issues a direct transport command, defers a pause through `MediaFocusPauseBridge`, or hands a package with no session left to `MediaResumeReconnect`. Identity is the package, the last-played one is persisted, and a refusal always names its reason from one vocabulary, which `MediaKeyDiagnostics` keeps in a ring that `MediaKeyReport` renders onto the «Сервис» screen — this firmware's `log.tag=M` swallows the tag, so an owner without host ADB has no other way to read it. See [shortcuts-automation-findings.md](shortcuts-automation-findings.md) for the normative contract. |
| `feature.defaultapps` | The three stock AutoVoice roles behind «Приложения»: navigation, music and video. `DefaultAppRoleRepository` reads and writes the `PersonBean` rows directly through `ContentResolver` from the app UID, with no ADB path; `DefaultAppsPolicy` models the exact `SETTING` keys and a first-run ordering, not an allowlist, so any installed application with a launcher may be persisted. `Settings.Global.byd_map_package` is deliberately not part of it. See [shortcuts-automation-findings.md](shortcuts-automation-findings.md). |
| `feature.panel` | The colours every canvas panel draws with, resolved from `design.DenzaPalette` — the palette read out of this car's own SystemUI — plus the small blend and alpha helpers around them. It holds no literals: the six values it used to copy by hand had already drifted against a second copy in the split picker. |
| `feature.simulcast` | The casting geometry, split out of the accessibility service: `ScreenTarget` names a DiShare receiver, `SimulcastVideoSizeResolver` matches it to a physical Android display and keeps that target's pixel viewport, and `SimulcastVideoBoundsResolver` computes the centered aspect-fit rectangle around DiShare's firmware-enforced minimum 16:9 stream. Unit-tested; the rear-screen case is not yet live-car verified. |
