...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation, a manually toggled processed DVR view, HUD guidance, explicit stock-IVI split sessions, and passenger-screen app installation. | **Active.** Version `0.5.3`; one installed APK contains both the permanent Denza Apps launcher and the toggle-controlled Split Screen alias plus its picker/host. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, the former contextual split route, and FSE installation have been exercised on the author's car. The replacement two-picker split startup and native hosting are live-verified; the final one-APK product packaging is locally verified, while dismissal, divider, projection, and one-APK live acceptance remain. Rapid left-to-right mirror switching remains unsafe. |
...
| `feature.adb`, `adb/DenzaLocalAdb` | Hidden ADB Rescue flow and the canonical passive client for Denza Apps. Feature code cannot enqueue authorization prompts; diagnostics can submit one persisted manual request. Queue draining remains disabled until the vehicle acceptance gate in [adb-authorization-recovery.md](adb-authorization-recovery.md) passes. |
