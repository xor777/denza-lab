...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation and HUD guidance, explicit stock-IVI split sessions, a swipeable bottom panel (trip/spectrum plus vehicle instruments), native weather adaptation, passenger-screen app installation, and a hidden stock-settings Russian locale toggle. | **Active.** Version `0.5.5`; one installed APK contains both the ordinary Denza Apps launcher and the toggle-controlled Split Screen alias plus its picker. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, trip inputs, native weather refresh, the former contextual split route, and FSE installation have been exercised on the author's car. Split startup, restore, divider/edge behavior and Home isolation have live history, but the latest driver run reopened their reliability acceptance and the feature was rebuilt on the contract core on 2026-08-23; the normative scenarios, the current gaps and the live-acceptance protocol are recorded in [split-screen-product-contract.md](split-screen-product-contract.md), and that rebuilt core is still waiting for its run on the car. The vehicle-instrument page and Russian locale toggle are locally verified but not yet accepted on the car. The processed front-DVR renderer remains compiled but product-disabled; rapid left-to-right mirror switching remains unsafe. |
...
| `feature.trip` | Compile-time-enabled trip/spectrum panel, now the first page of the swipeable bottom panel. While visible, it uses standard Android gravity/gyro/accelerometer sensors at approximately 30 Hz, GNSS at approximately 1 Hz, the validated Yandex guidance runtime, session-0 `Visualizer`, and the active media session. Sensors, GNSS, audio capture, and rendering stop when the panel is detached or the Activity is paused; swiping to another page stops only the drawing, so the trip keeps recording. The older mirror-toy/compass/journey-thread presentation is compiled but hidden by `LEGACY_INSTRUMENTS=false`. |
| `feature.vehicle` | Second page of the bottom panel: battery fill and state of charge, traction voltage on a narrow-span gauge, a temperature stack (pack, inverters, all three drive motors) with normal bands, and consumption per 200 m. Each block leads with the one figure that moves; quiet numbers are supporting lines whose colour, not size, raises a hand. Reads a 44-entry `autoservice` allowlist through the passive local ADB client — read transacts only, no `BYDAUTO_*` permission, no `app_process` proxy — batching one command per sweep with an echoed index per call so an unanswered feature id cannot shift the other answers. Hot signals refresh at panel cadence and are never carried over stale; cold signals refresh every 10–30 s. Values that cannot be true for their unit are dropped rather than drawn, and an untrusted ADB key leaves the page empty with the reason. The hub only opens a shell after a panel page has been opened once. Distance for the histogram comes from the vehicle odometer, so the page needs no location permission; the live figure is a rolling 300 m window that stops at a standstill, where energy per kilometre has no value. Reviewed on the car 2026-08-23; tyres and cabin climate were removed as duplicates of native displays. |
| `feature.vehicle` engine page | Third page of the bottom panel and the combustion half of the hybrid: revolutions, engine state, generation into the pack, and eight fluid lamps folded from sixteen feature ids. Its 21 signals join the poll only while the page is on screen — a full sweep costs 266–315 ms without them and 468–587 ms with them. Designed for its resting state: the car drives on electricity, so zero revolutions and eight quiet lamps are the normal view, drawn muted rather than bright. There is no engine coolant temperature on this firmware — three candidate ids on six devices all returned sentinels — so the overheat lamp is present and the gauge a driver expects is not. Calibrated against a measured start/stop cycle: revolutions are real rpm (1619 start peak, 1321 generation set-point), generation is kilowatts (it mirrors pack power exactly), engine state is 0 stopped / 3 running and generation state 2 is the shutdown rather than generation. The generation dial is square-root, since idle generation is about 10 kW against a much higher ceiling. Shows inverter temperature as the generation path's only thermal reading — it moved 26 to 32 degrees across a minute of generating. |
