...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation, a manually toggled processed DVR view, HUD guidance, stock IVI split routing, and passenger-screen app installation. | **Active.** Version `0.5.0`; Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, contextual stock split routing, and FSE installation have all been exercised on the author's car. Rapid left-to-right mirror switching remains unsafe. |
