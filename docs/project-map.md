...
| `feature.vehicle` | Parked vehicle-instrument implementation, not mounted by the current `DenzaAppsScreen`. Its last product layout showed battery fill/state of charge, traction voltage, pack/inverter/motor temperatures, and consumption per 200 m from a bounded `autoservice` allowlist through passive local ADB. The 2026-08-23 measurements remain valid implementation evidence; promotion back into the screen requires a separate product/design decision. |
| `feature.vehicle` engine page | Parked combustion-instrument implementation, not mounted by the current `DenzaAppsScreen`. Its last layout showed revolutions, engine/generation state, inverter temperature, and eight fluid lamps; the measured poll cost and 2026-08-23 calibration remain documented evidence rather than a claim about the current UI. |
