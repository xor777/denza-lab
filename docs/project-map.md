...
| `feature.cloud` | The «Облако» tile: the car's own cloud client (`cloudmanager`) kept online over Wi-Fi, so the official Denza phone app sees the car. `CloudLinkService` watches validated Wi-Fi and `CloudLinkController` translates it for the stock client through the shell - the `double_apn` profile, `notify_nw(4)` on Wi-Fi and `-5` when it has stayed gone - with `CloudLinkCore` deciding when, including bounded repeats after the stock receiver closes the gate itself. A second switch is the car's own `byd_off_wifi_switch` (Wi-Fi kept on through ACC-off), read back from the car. Off never touches the car on its own. Built and unit-tested on 2026-09-23; not yet installed or live-tested. Contract: "Denza Apps «Облако» tile" in [telematics-findings.md](telematics-findings.md). |
