...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation and HUD guidance, explicit stock-IVI split sessions, a bottom trip/spectrum strip, native weather adaptation, passenger-screen app installation, Shortcuts default-app role selection, and a hidden stock-settings Russian locale toggle. | **Active.** Version `0.6.0-alpha`; one installed APK contains the ordinary Denza Apps launcher, the toggle-controlled Split Screen alias and picker, and the windowless default-navigation proxy. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, trip inputs, native weather refresh, the former contextual split route, and FSE installation have been exercised on the author's car. Split startup, restore, divider/edge behavior and Home isolation have live history, but the latest driver run reopened their reliability acceptance and the feature was rebuilt on the contract core on 2026-08-23; the normative scenarios, the current gaps and the live-acceptance protocol are recorded in [split-screen-product-contract.md](split-screen-product-contract.md), and that rebuilt core is still waiting for its run on the car. The Russian locale toggle and built-in navigation proxy are locally verified but not yet accepted on the car. Rapid left-to-right mirror switching remains unsafe. |
...
| Component | Status |
| --- | --- |
| `MainActivity`, `ui/DenzaAppsScreen` | Adaptive Compose shell with eleven dashboard tiles, including the `Приложения` entry for navigation, music and video defaults used by stock Shortcuts. Its panel reads and writes the three AutoVoice PersonBean roles directly through `ContentResolver` (live-proven 2026-09-03). Music and video remain direct PersonBean packages; a non-stock navigation selection is represented there by `dev.denza.apps`, while an atomically committed cross-process file is the logical target displayed by the panel. No background role listener is added: AutoVoice cold-starts the package's single `MAIN + INFO` entry, a windowless trampoline in `:picker`. The generic AutoVoice app-open `FROM` marker still routes to `MainActivity`; a plain map-role launch routes to the configured navigator. Every installed launcher remains selectable, including Denza Apps itself. Fullscreen keeps the original landscape dashboard; the measured 2/3 pane exposes that dashboard through horizontal scrolling, while the measured 1/3 pane stacks every tile in one vertical list. Pickers use adaptive grids in the narrow pane, and the trip spectrum spans the pane above vertically stacked trip figures. Below the tiles sits the one always-present trip/spectrum strip; the former `BottomPanelPager` and its vehicle/engine pages were retired and deleted on 2026-08-27. Settings use standard Material 3 switches, segmented selectors, and action buttons; selected Simulcast apps stay in one compact horizontal summary. The Navigation tile includes an opt-in steering-wheel control: every ★ press independently requests one immediate navigation action, with no double-press behavior. Its picker also offers this app's own instruments (`Приборы`) beside the navigators; that choice has one placement, so the tile drops its placement row instead of showing three dead cells. Attention states show a concrete instruction and reuse existing action slots; technical diagnostics remain hidden in Help. |
...
| `SimulcastBootReceiver` | Non-exported receiver that invokes runtime recovery only after boot or APK replacement. DiShare dialog visibility comes from the already-required accessibility observer, not app-spoofable vendor broadcasts. |
...
| `feature.vehicle` | Vehicle-telemetry backend used only by `feature.cluster.dashboard`: a bounded `autoservice` allowlist through passive local ADB, the consumption journal, and the calibrated electrical/combustion readings and lamps. The cluster dashboard owns the polling lifecycle and reads the full signal set it needs while visible; its consumption display is fixed at 3 km. The former head-unit `VehiclePanelView`, `EnginePanelView`, their renderers, and `BottomPanelPager` were retired and deleted on 2026-08-27, with their 2026-08-22/23 measurements retained in [vehicle-data-findings.md](vehicle-data-findings.md) as historical evidence. |
