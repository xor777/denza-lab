...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation and HUD guidance, explicit stock-IVI split sessions, trip/spectrum display, native weather adaptation, passenger-screen app installation, and a hidden stock-settings Russian locale toggle. | **Active.** Version `0.5.4`; one installed APK contains both the ordinary Denza Apps launcher and the toggle-controlled Split Screen alias plus its picker/host. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, trip inputs, native weather refresh, the former contextual split route, and FSE installation have been exercised on the author's car. One-APK split startup, basic native divider movement, toggle shutdown, standalone-picker navigation projection/return, and Denza Apps ordinary launch/Back are live-verified. A divider ownership/reconciliation boundary case remains open. The Russian locale toggle is locally verified but not yet exercised on the car. The processed front-DVR renderer remains compiled but product-disabled; rapid left-to-right mirror switching remains unsafe. |
...
| Component | Status |
| --- | --- |
| `MainActivity`, `ui/DenzaAppsScreen` | Adaptive Compose shell with three main cards (Navigation, Simulcast, Mirrors) and three compact cards (Split screen, HUD hints, passenger-screen installation). Fullscreen keeps the original landscape dashboard; the measured 2/3 pane exposes that dashboard through horizontal scrolling, while the measured 1/3 pane stacks every card in one vertical list. Pickers use adaptive grids in the narrow pane, and the trip spectrum spans the pane above vertically stacked trip figures. Main-card settings use standard Material 3 switches, segmented selectors, and action buttons; selected Simulcast apps stay in one compact horizontal summary. The Navigation card includes an opt-in steering-wheel control: one ★ press controls navigation. The double-press DVR trigger remains compiled, but its request is ignored while `ClusterDvrFlag.ENABLED=false`. Attention states show a concrete instruction and reuse existing action slots; technical diagnostics remain hidden in Help. |
...
| `feature.navigation` | Public app-owned virtual display, fixed shell operations for task movement, an installed-app picker, saved full/left/center/right placement, and an opt-in accessibility-filtered steering-wheel key binding. A 500 ms press-sequence recognizer preserves the normal navigation action and distinguishes the compiled double-★ DVR path, whose request is currently ignored by the retired feature flag. Projection, live layout switching, steering-wheel project/return, and HUD guidance while projected are live-car verified. Automatic following of the stock Map mode remains implemented but its unfinished UI control is hidden. |
| `feature.split` | Explicit «Разделить экран» launcher plus two standalone, resizeable Denza picker tasks, one per BYD root. Exact `START_IVI_PRIMARY` / `START_IVI_SECOND` categories create and target the native roots without transaction 115; unresizeable Launcher3/SR bootstrap tasks are not retained as hosts. The dedicated accessibility observer only primes a background shell when the stock picker appears; it creates no window and performs no task mutation until BYD reports balanced area `3`, the active divider pointer is released, and that released state is stable. A picker tap launches exactly the chosen app as an app-owned task above that picker, including Denza Apps itself; ordinary apps use independent multiple tasks, while `singleTask`/`singleInstance` duplicates remain rejected by their Android contract. `SplitAppHostActivity` is not on the active selection path, so SmartMulti's native divider overlay sees the real app identity. `MainActivity` follows ordinary task/Back behavior and never calls the split coordinator or mutates its persisted slots. Native configuration changes and visible-picker callbacks reconcile the automaton with live root/task topology because BYD can swap app tasks between stable picker hosts during divider resize. A transient root containing both picker bases is ignored until delayed reconciliation restores one base per root and adopts the settled app ownership. The last pair and automaton ownership are persisted, stale full-IVI tasks are not mistaken for live peer windows, and task cleanup is component-validated through the local-ADB shell helper. Direct category placement, picker dismissal/reopen, divider movement including the coalesced-base boundary, collapse/reopen restoration, toggle shutdown, and standalone-picker navigation projection/return are live-proven. Simulcast still needs its own focused cross-feature acceptance pass. |
...
- `denza-apps` is the single active Denza feature app. Simulcast, migrated camera
  ...
  trip/spectrum panel, native weather adaptation, and passenger-screen
  installation share one UI and runtime state model.
