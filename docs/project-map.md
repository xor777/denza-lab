...
| `apps/denza-apps/` | `denza-apps` | Simulcast, side-camera mirrors, navigation and HUD guidance, explicit stock-IVI split sessions, trip/spectrum display, native weather adaptation, and passenger-screen app installation. | **Active.** Version `0.5.3`; one installed APK contains both the permanent Denza Apps launcher and the toggle-controlled Split Screen alias plus its picker/host. Compose landscape shell, self-recovery, one display resolver, and one shared cluster scene. Mirror cycles, selectable navigation layouts, HUD guidance, trip inputs, native weather refresh, the former contextual split route, and FSE installation have been exercised on the author's car. One-APK split startup, saved-pair restore, native divider movement, full-screen control entry/return, toggle shutdown, and standalone-picker navigation projection/return are live-verified. The processed front-DVR renderer remains compiled but product-disabled; rapid left-to-right mirror switching remains unsafe. |
