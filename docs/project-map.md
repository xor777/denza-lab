...
| `feature.mirrors` | Migrated AVC renderer and stock-window monitor. Uses the shared local ADB client, keeps verified Mirrors geometry/image treatment, and has no probe dependency. It holds a demand lease on the vehicle-signal hub for the confirmed mode (bounded shadow diagnostics only) and the raw lever phase. The stock-window observer is the only Show authority. A raw onset pulse can only tear down an active Denza camera of the other side; a pulse for the side already starting or showing is ignored; the phase never selects a side, and hub unavailability never touches the camera. After a preempt the reducer reopens only on two consecutive observations of the other side with the runtime idle and vendor teardown complete; the preempted side's stale window never reopens; neutral recovers. The two earlier CAN-gated contracts were retired on 2026-09-04 after they made the camera appear randomly (see [vehicle-data-findings.md](vehicle-data-findings.md#window-only-show-onset-only-teardown-2026-09-04-late)); the window-only contract is unit- and mutation-covered but not yet driven. |
