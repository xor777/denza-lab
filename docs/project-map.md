...
| `feature.mirrors` | Migrated AVC renderer and stock-window monitor. Uses the shared local ADB client, keeps verified Mirrors geometry/image treatment, and has no probe dependency. It consumes confirmed mode and transient raw onset events through the demand-driven vehicle-signal hub. A raw edge can only preempt the old surface; a later same-epoch confirmed side plus a stable matching stock window may reopen after full vendor teardown. Unknown/unavailable input requires neutral. One stationary direct left-to-right canary and one separated left-off-right cycle are live-accepted; the exact transient-ambiguity wait branch has unit coverage but did not recur in the final live run. |
