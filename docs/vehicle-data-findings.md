...
| **Vehicle speed is polled hot, and nothing draws it** | `VehicleSignal.VEHICLE_SPEED` (`0x94400008`, dev 1013, tx 7) | The consumption log integrates pack power into a bucket only above `ConsumptionLog.STANDING_KMH` = 0.5 km/h, and a sweep that got no answer for it counts as moving. The question is asked of the interval, so a speed ten seconds stale would file a minute of the engine charging on P into the next hundred metres of road - the defect the rule exists to stop, arriving by the back door. It rides in the batch the sweep already sends past this bus, and the trip's ledger keeps the standing joules (`docs/energy-display-contract.md` §2.2) |
